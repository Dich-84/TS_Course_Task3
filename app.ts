const posts = [
  ...
  const normalizeData: Function = (unnormalizedData: Post[]): normalizedData => {
    let normalizedData: normalizedData = {
        byId: {},
        allIds: []
    }

    for (let obj of unnormalizedData) {
        let key = obj.id
        normalizedData.byId[key] = obj
        normalizedData.allIds.push(key)
    }
